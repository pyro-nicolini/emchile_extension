...
module.exports = async function (fastify, opts) {
  ...
  fastify.post('/', async (request, reply) => {
    ...
    const query = `INSERT INTO quotations (${keys.join(', ')}) VALUES (${placeholders}) RETURNING *`;
    try {
  ...
  fastify.put('/:id', async (request, reply) => {
    ...
    values.push(id);
    const query = `UPDATE quotations SET ${setClause} WHERE id = $${values.length} RETURNING *`;
    try {
      const { rows } = await db.query(query, values);
      if (!rows.length) return reply.status(404).send({ error: 'Recurso no encontrado' });
      return { data: rows[0] };
  ...
  fastify.delete('/:id', async (request, reply) => {
    const { id } = request.params;
    const { rows } = await db.query('DELETE FROM quotations WHERE id = $1 RETURNING id', [id]);
    if (!rows.length) return reply.status(404).send({ error: 'Recurso no encontrado' });
